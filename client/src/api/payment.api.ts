...
import { CashCheckoutBody, Checkout, FullCheckoutBody, InitPartialCheckoutBody, PartialCheckoutBody, PartialTransaction, PartialTransactions, RefundBody } from "@/types/payment";
...
export default class PaymentApi {
    ...
    static async initializePartialCheckout(body: InitPartialCheckoutBody): Promise<FetchResponse<PartialTransactions>> {
        return await fetch({
    ...
    static async cashCheckout(body: CashCheckoutBody): Promise<FetchResponse<Transaction>> {
        return await fetch({
            url: `${apiBaseUrl}/payments/cash`,
            method: HTTPMethod.POST,
            headers: defaultHeaders,
            body: JSON.stringify(body)
        });
